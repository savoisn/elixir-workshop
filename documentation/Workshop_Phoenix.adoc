...
==== Windows concerns
you need some sort of compiler instaled for bcrypt to work.
On Windows if you don't already have one installed, you can follow https://github.com/frenshkosh[frenshkosh^] way of fixing the problem
...
Thanks https://github.com/frenshkosh[frenshkosh^] for the solution :D
...
We want a chat, with Rooms to chat inside !
So we want *messages* from *users* inside *rooms*
Sounds doable !
...
And then the Message in the same context but with a relation to the Room and the User
Each message is from a user and is going in a room :D
...
----
We check if the User with email nicolas.savois@gmail already exists.
If not we create it we are using the create_user that hash our password. 
Don't do that in production... the password are in plain text...
...
They tested it ! https://phoenixframework.org/blog/the-road-to-2-million-websocket-connections[Here]
...
2 methods : 
- join : let's you controller who joins what !
- handle_in/3 : handle incomming message

What we will do it use a handle_in for new message and rebroacast that message accross the channel.
...
The server is just moving things around get messages and rebroacasting it.

Need an interface to test that.
In the main page of our app let's clean and play with that

First enable the channel connection from the webpage
It's done with some kind of Javascript - I see your joyfull look... don't hide it !
...
Now edit the socket.js to implement a minimal logic to test the channel.
We will only push a message after the browser is connected and listen for message, we display everything in the console to validate the round trip
...
`channel.on` will "listen" on message topic for a payload and act accordingly
`channel.push` let us push on messages topic with a payload
...
.lib/slang_web/templates/pages/index.html.eex
[source,html,subs="+quotes,+macros"]
----
...
Listen for new_msg broadcast and add it to the messagesContainer

And we are done !

startup 2 different browser login with 2 differents user and start chat with yourself !

you can checkout the finished version of the code here on tag `simplistic_chat_finished`
...
.lib/controllers/room_controller.ex
[source,javascript,subs="+quotes,+macros"]
----
...
----
We just query the room by its id and render that via the json helper function instead dof the ussual render function +
